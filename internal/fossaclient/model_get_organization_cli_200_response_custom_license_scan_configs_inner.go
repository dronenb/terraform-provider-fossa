/*
FOSSA API

OpenAPI Specification for public FOSSA APIs

API version: 4.34.81
Contact: support@fossa.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fossaclient

import (
	"encoding/json"
)

// checks if the GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner{}

// GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner struct for GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner
type GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner struct {
	// custom license scan name
	Name *string `json:"name,omitempty"`
	// regular expression for the custom license scan
	MatchCriteria *string `json:"matchCriteria,omitempty"`
}

// NewGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner instantiates a new GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner() *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner {
	this := GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner{}
	return &this
}

// NewGetOrganizationCLI200ResponseCustomLicenseScanConfigsInnerWithDefaults instantiates a new GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetOrganizationCLI200ResponseCustomLicenseScanConfigsInnerWithDefaults() *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner {
	this := GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) SetName(v string) {
	o.Name = &v
}

// GetMatchCriteria returns the MatchCriteria field value if set, zero value otherwise.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) GetMatchCriteria() string {
	if o == nil || IsNil(o.MatchCriteria) {
		var ret string
		return ret
	}
	return *o.MatchCriteria
}

// GetMatchCriteriaOk returns a tuple with the MatchCriteria field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) GetMatchCriteriaOk() (*string, bool) {
	if o == nil || IsNil(o.MatchCriteria) {
		return nil, false
	}
	return o.MatchCriteria, true
}

// HasMatchCriteria returns a boolean if a field has been set.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) HasMatchCriteria() bool {
	if o != nil && !IsNil(o.MatchCriteria) {
		return true
	}

	return false
}

// SetMatchCriteria gets a reference to the given string and assigns it to the MatchCriteria field.
func (o *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) SetMatchCriteria(v string) {
	o.MatchCriteria = &v
}

func (o GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.MatchCriteria) {
		toSerialize["matchCriteria"] = o.MatchCriteria
	}
	return toSerialize, nil
}

type NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner struct {
	value *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner
	isSet bool
}

func (v NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) Get() *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner {
	return v.value
}

func (v *NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) Set(val *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) {
	v.value = val
	v.isSet = true
}

func (v NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) IsSet() bool {
	return v.isSet
}

func (v *NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner(val *GetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) *NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner {
	return &NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner{value: val, isSet: true}
}

func (v NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetOrganizationCLI200ResponseCustomLicenseScanConfigsInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


