/*
FOSSA API

OpenAPI Specification for public FOSSA APIs

API version: 4.34.81
Contact: support@fossa.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fossaclient

import (
	"encoding/json"
)

// checks if the CustomLicenseScanConfig type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CustomLicenseScanConfig{}

// CustomLicenseScanConfig struct for CustomLicenseScanConfig
type CustomLicenseScanConfig struct {
	// custom license scan name
	Name *string `json:"name,omitempty"`
	// regular expression for the custom license scan
	MatchCriteria *string `json:"matchCriteria,omitempty"`
}

// NewCustomLicenseScanConfig instantiates a new CustomLicenseScanConfig object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCustomLicenseScanConfig() *CustomLicenseScanConfig {
	this := CustomLicenseScanConfig{}
	return &this
}

// NewCustomLicenseScanConfigWithDefaults instantiates a new CustomLicenseScanConfig object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCustomLicenseScanConfigWithDefaults() *CustomLicenseScanConfig {
	this := CustomLicenseScanConfig{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *CustomLicenseScanConfig) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomLicenseScanConfig) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *CustomLicenseScanConfig) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *CustomLicenseScanConfig) SetName(v string) {
	o.Name = &v
}

// GetMatchCriteria returns the MatchCriteria field value if set, zero value otherwise.
func (o *CustomLicenseScanConfig) GetMatchCriteria() string {
	if o == nil || IsNil(o.MatchCriteria) {
		var ret string
		return ret
	}
	return *o.MatchCriteria
}

// GetMatchCriteriaOk returns a tuple with the MatchCriteria field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CustomLicenseScanConfig) GetMatchCriteriaOk() (*string, bool) {
	if o == nil || IsNil(o.MatchCriteria) {
		return nil, false
	}
	return o.MatchCriteria, true
}

// HasMatchCriteria returns a boolean if a field has been set.
func (o *CustomLicenseScanConfig) HasMatchCriteria() bool {
	if o != nil && !IsNil(o.MatchCriteria) {
		return true
	}

	return false
}

// SetMatchCriteria gets a reference to the given string and assigns it to the MatchCriteria field.
func (o *CustomLicenseScanConfig) SetMatchCriteria(v string) {
	o.MatchCriteria = &v
}

func (o CustomLicenseScanConfig) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CustomLicenseScanConfig) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.MatchCriteria) {
		toSerialize["matchCriteria"] = o.MatchCriteria
	}
	return toSerialize, nil
}

type NullableCustomLicenseScanConfig struct {
	value *CustomLicenseScanConfig
	isSet bool
}

func (v NullableCustomLicenseScanConfig) Get() *CustomLicenseScanConfig {
	return v.value
}

func (v *NullableCustomLicenseScanConfig) Set(val *CustomLicenseScanConfig) {
	v.value = val
	v.isSet = true
}

func (v NullableCustomLicenseScanConfig) IsSet() bool {
	return v.isSet
}

func (v *NullableCustomLicenseScanConfig) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCustomLicenseScanConfig(val *CustomLicenseScanConfig) *NullableCustomLicenseScanConfig {
	return &NullableCustomLicenseScanConfig{value: val, isSet: true}
}

func (v NullableCustomLicenseScanConfig) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCustomLicenseScanConfig) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


